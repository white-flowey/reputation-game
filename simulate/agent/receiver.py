...
class Receiver:
    ...
    def receive(self, topic: int, speaker: int, statement: Info, blush: bool) -> dict:
        ...
        trust = self.compute_trust(
            topic=topic, speaker=speaker, statement=statement, blush=blush
        )

        Ispeaker = self.interprete_speaker(
            speaker=speaker, topic=topic, statement=statement
        )
        if speaker != topic:
            Itopic = self.interprete_topic(
                speaker=speaker, topic=topic, statement=statement
            )
        ...
        self.a.Updater.update_ToM(
            trust=trust, speaker=speaker, topic=topic, statement=statement
        )
        return reception

    def compute_trust(
        self, topic: int, speaker: int, statement: "Info", blush: bool
    ) -> float:
        ...
        if (
            self.a.character in ["villager_2", "villager_1"]
            and most_trusted_agend is not None
            and speaker != self.a.id
        ):
        ...
        if self.a.listening and speaker == topic and statement.mean < assumed_honesty:
            return 1

        surprise_factor = self.handle_surprise(statement=statement, topic=topic)
        denominator = surprise_factor * (
            1 - self.a.conf("BLUSH_FREQ_LIE") * (1 - assumed_honesty)
        )
    ...
    def handle_surprise(self, statement: Info, topic: int) -> float:
        ...
        """
        if not self.a.listening or self.a.uncritical:
            return 1
    ...
    def interprete_speaker(self, speaker: int, topic: int, statement: "Info") -> dict:
        ...
        if speaker == topic:
            Inew = Ift.get_info_difference(
                P=statement, Q=self.a.Iothers[speaker][topic]
            )
